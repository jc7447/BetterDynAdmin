(function($) {
  ...
  var BDA_PIPELINE = {
    ...
    setupPipelineManagerPage: function() {
      ...
      $("h2:contains('Pipeline Chains')").append("<div class='popup_block' id='pipelinePopup'>" + "<div><a href='javascript:void(0)' class='close'><i class='fa fa-times'></div>" + "<div><h3></h3></div></i></a>" + "<button id='schemeOrientation'>Switch orientation <i class='fa fa-retweet'></button>" + "<div id='pipelineScheme'></div></div>");
      ...
      $pipelineTable.find("tr:gt(0)").prepend('<td align="center"><i class="fa fa-arrow-down link show-xml show-xml-close"></i><i class="fa fa-arrow-right link show-xml show-xml-open"></i></td>').append("<td align='center'><i class='fa fa-code link show-xml'></i></td><td align='center'><i class='fa fa-eye link show-graph'></i>" + "<sup style='font-size:8px'>&nbsp;BETA</sup></td>");
      ...
      processRepositoryXmlDef("definitionFile", function($xmlDef) {
        ...
        $pipelineTable.find('tr').each(function(index, elem) {
          var $elem = $(elem);
          var chainName = $elem.find("td:eq(1)").text();
          $elem.attr("id", chainName);
          $elem.find('.show-xml').click(function() {
    ...
    createNodesAndEdges: function(chainName) {
      ...
      $chainDef.find('pipelinelink').each(function(pipelinelinkIndex, pipelinelinkElement) {
        ...
        nodes.add({
          id: pipelinelinkIndex,
          label: pipelineLinkName,
          name: pipelineLinkName,
          pipelineLinkPath: pipelineLinkPath
        });
      });
      ...
      $chainDef.find('pipelinelink').each(function(pipelinelinkIndex, pipelinelinkElement) {
        ...
        $(pipelinelinkElement).find('transition').each(function() {
          ...
          edges.add({
            from: BDA_PIPELINE.findNodeId(nodes, pipelineLinkName),
            to: BDA_PIPELINE.findNodeId(nodes, transitionName),
            arrows: 'to',
            label: $(this).attr("returnvalue")
          });
        });
